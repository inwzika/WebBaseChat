...
// Auto-enable proxy when no API URL is provided, or explicitly enabled
const USE_PROXY = (import.meta.env.VITE_USE_PROXY as string | undefined) === "true" || !API_URL;
// When proxying, use same-origin so cookies are first-party (baseURL="/api")
const axiosClient = axios.create({ baseURL: USE_PROXY ? "/api" : API_URL });
